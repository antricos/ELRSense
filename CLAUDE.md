...
This cap has to hold across *different* catalog entries too, not just within one: `gps_m100_5883` and `generic_nmea_gps` are two separate sensors (same underlying NMEA GGA/RMC parser, duplicated under `firmware/sensors/generic_nmea_gps/` and renamed, so a user whose module isn't specifically the M100-5883 doesn't have to pick it in the UI) that both emit CRSF 0x02 — `app.js`'s `frameGroupSensors()`/`frameGroupHasInstance()` disable a sensor's "+ Add" button whenever any other `maxInstances: 1` sensor sharing its `frame` value already has an instance, not just when the sensor's own count is at its own cap. Any future source_id-less sensor sharing a frame with an existing one needs this same check, not just `maxInstances: 1` on its own entry.
