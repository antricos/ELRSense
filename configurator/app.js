/**
 * UI wiring: board dropdown, baud dropdown, per-category sensor-type picker
 * + "+ Add" button, a combined instance list per category (pin dropdown for
 * pin-role sensors, none for shared-I2C-bus ones), generate button.
 */
...
// Which sensor type the category's "+ Add" dropdown currently targets, per
// SENSOR_GROUPS id. Lives outside `state` (the actual added-instance data)
// since it's pure UI selection and must survive the full-DOM rebuild every
// renderSensors() call does.
const groupSelection = {};

function renderInstanceRow(sensor, inst, index, ownCount) {
    const row = document.createElement("div");
    row.className = "instance-row";

    const label = document.createElement("span");
    label.className = "instance-label";
    label.textContent = sensor.icon ? `${sensor.icon} ${sensor.name}` : sensor.name;
...
function renderCategoryCard(group, sensors) {
    ...
    // Keep the previous pick if it's still a member of this group (e.g.
    // after adding/removing an instance elsewhere), else default to the
    // first sensor type.
    if (!sensors.some((s) => s.id === groupSelection[group.id])) groupSelection[group.id] = sensors[0].id;
    const selected = SENSORS[groupSelection[group.id]];

    if (sensors.length > 1) {
        const select = document.createElement("select");
        select.className = "sensor-type-select";
        for (const sensor of sensors) {
            const opt = document.createElement("option");
            opt.value = sensor.id;
            opt.textContent = sensor.icon ? `${sensor.icon} ${sensor.name}` : sensor.name;
            if (sensor.id === selected.id) opt.selected = true;
            select.appendChild(opt);
        }
        select.addEventListener("change", () => {
            groupSelection[group.id] = select.value;
            renderSensors();
        });
        ...
        name.textContent = selected.icon ? `${selected.icon} ${selected.name}` : selected.name;
        header.appendChild(name);
    }

    const max = maxInstancesFor(selected);
    const atOwnMax = state[selected.id].length >= max;
    ...
    // the slot instead.
    const blockedBySibling = selected.maxInstances === 1 && frameGroupHasInstance(selected, true);
    const atMax = atOwnMax || blockedBySibling;
    const noPinsLeft = selected.pinRole && availablePins(selected.pinRole, null).length === 0;
    ...
    if (atMax || noPinsLeft) {
        ...
        addBtn.title = blockedBySibling
            ? `Only one ${selected.frame} sensor can be added at a time`
            : atMax ? "Maximum instances reached" : "No free pins left for this sensor type";
    }
    addBtn.addEventListener("click", () => {
        if (selected.pinRole) {
            const options = availablePins(selected.pinRole, null);
            if (!options.length) return;
            state[selected.id].push({ pin: options[0], config: defaultConfigFor(selected) });
        } else {
            state[selected.id].push(true);
        }
        renderSensors();
    });
    ...
    if (sensors.some((s) => s.usesI2c && !s.pinRole)) {
    ...
    for (const sensor of sensors) {
        const ownCount = state[sensor.id].length;
        state[sensor.id].forEach((inst, index) => {
            instList.appendChild(renderInstanceRow(sensor, inst, index, ownCount));
        });
    }
...
function renderSensors() {
    ...
    for (const group of SENSOR_GROUPS) {
        ...
        section.appendChild(renderCategoryCard(group, sensors));
